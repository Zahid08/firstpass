        ...
        <div class="step_choose_wrapper">
            <div class="step_choose_tab">
                <a href="javascript:history.back()" class="step_back_span back_ancor">
                    <img src="{!! url('frontend_assets/images/back-arrow.svg') !!}" alt=""><span>Back</span>
                </a>
                <div class="step_tab_img">
                    <img class="img-fluid" src="{!! url('frontend_assets/images/payment-tabs.svg') !!}" alt="">
                </div>
            </div>
        </div>
        <div class="payment_wrapper">

            {{--loaded cart info here--}}
                    ...
                    <div class="col-md-12">
                        <div class="your_cart_box">
                            <div class="cart_header">
                                ...
                                <img src="{!! url('frontend_assets/images/payment-cart-icon.svg') !!}" alt="">
                                <?php
                            ...
                            <div class="cart_body" style="min-height: 195px;">
                               @if($instructor) <h5>Your booking with {{ ($instructor)?ucfirst( $instructor->name ):'' }}:</h5>@else <h5 style="text-transform:none">You are adding new credit to your wallet.</h5> @endif
                                <ul class="list-unstyled">
                                    ...
                                    ?>
                                    @if($checkLesson)
                                    @foreach($resultLesson as $key=>$item)
                                        @php $totalBookingHour+=$item; @endphp
                                        <li class="@if($key>2) hidden-show @endif">{{$item}} Hour Lesson {{date('d-m-Y',strtotime($resultLessonScheduleDate[$key]))}}, {{$resultTimeSLot[$key]}}</li>
                                    @endforeach @endif

                                       <?php $existTest=false;?>
                                        @if($search->step_4 && $search_step_2->test_schedule_date && $search_step_2->test_schedule_date!='undefined' && $checkTest)
                                            <?php
                                            $existTest=true;
                                            // Given time range
                                            ...
                                            ?>
                                            <li id="drivingTestPackage">Driving Test Package: {{date('d-m-Y',strtotime($search_step_2->test_schedule_date))}}, {{$startTime->format('h:i a')}} pickup for {{$endTime->format('h:i a')}} a test start</li>
                                        @endif

                                        @if(count ($resultLesson)==0 && $existTest==false && (isset($_REQUEST['type']) && $_REQUEST['type']!='wallet'))
                                            <p style="font-size: 16px;">No Booking Yet</p>
                                        @endif

                                </ul>
                                @if(count($resultLesson)>3)
                                ...
                                <div class="pack_price_list">
                                    ...
                                    <div class="dl_pkg_total">
                                        ...
                                        <div class="pkg_price_total">
                                            <span>Total</span>
                                            <input type="hidden" value="{{ $previousRate }}" id="drivingLessonPrice">
                                            <span>$<span id="totalPriceCalculations">{{$total}}</span></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="learners_form_wrapper overflow-hidden">
                    ...
                    <div class="form_body payment_form_body">
                        ...
                        <form class="row g-3" method="post" id="payment-form">
                            ...
                            <input type="hidden" value="{{$discount}}" name="dis" id="discountAMount">
                            <input type="hidden" value="{{$hour}}" name="hr" id="totalQtyHour">
                            <input type="hidden" value="{{ $previousRate }}" name="hourly_rate" id="hourly_rate">
                            <input type="hidden" value="{{ $test_package->price }}" name="test_price" id="test_price">
                            <input type="hidden" value="{{ $totalBookingHour }}" name="current_booking_hour" id="current_booking_hour">
                            ...
                            <input type="hidden" value="{{ isset($_REQUEST['type'])?$_REQUEST['type']:''}}" id="type" name="type"/>


                            <div class="col-md-6 col-6">
                                <label for="fname" class="form-label">First Name*</label>
                                <input style="pointer-events: none;  background: #8080800d;" class="form-control" type="text" name="name" @if($user) value="{{ $user->name }}" @endif>
                            </div>
                            <div class="col-md-6 col-6">
                                <label for="lname" class="form-label">Last Name*</label>
                                <input style="pointer-events: none;  background: #8080800d;" class="form-control" type="text" name="lname" @if($user) value="{{ $user->lname }}" @endif>
                            </div>
                            <div class="col-md-12">
                                <label for="address" class="form-label">Address*</label>
                                <input id="searchTextField" class="form-control" type="text" name="address" value="{{ $address }}" >
                                <small class=" address_hint" data-original-title="" data-toggle="tooltip" data-title="Address details"> <i class="fa fa-address-book"></i> <strong> Detail: </strong> city: {{ @$address_detail->city }}, suburb: {{ @$address_detail->suburb }} </small>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="custome-radio-wrapper d-flex justify-content-between payment-cards-row">
                                    ...
                                    <img src="{!! url('frontend_assets/images/payment-cards-img.png') !!}" height="22" alt="" class="ms-auto">
                                </div>
                                        ...
                                        <div class="custome-radio-wrapper">
                                            <div class="form-check form-check-inline mt-0">
                                                <label class="mb-0 form-check-label form-label" onclick="paymentChecker(this)" for="paypal_pay">
                                                    <img src="{!! url('frontend_assets/images/logos/paypal.png') !!}" alt="" class="paypal">
                                        ...
                                        <div class="custome-radio-wrapper">
                                            <div class="form-check form-check-inline mt-0">
                                                <label class="mb-0 form-check-label form-label" onclick="paymentChecker(this)" for="after_pay">
                                                    <img src="{!! url('frontend_assets/images/logos/afterPay-1.png') !!}" alt="" class="afterpay_icon">
                                        ...
                                        <div class="custome-radio-wrapper zip_icon">
                                            <div class="form-check form-check-inline mt-0">
                                                <label class="mb-0 form-check-label form-label" onclick="paymentChecker(this)" for="zip_pay">
                                                    <img src="{!! url('frontend_assets/images/logos/zip-pay-1.png') !!}" alt="">
