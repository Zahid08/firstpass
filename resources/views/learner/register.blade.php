        ...
        <div class="step_choose_wrapper">
            <div class="step_choose_tab">
                ...
                <div class="step_tab_img">
                    <img class="img-fluid" src="{!! url('frontend_assets/images/step-tabs-3.svg') !!}" alt="">
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="col-md-6">
                    <div class="your_cart_box">
                        ...
                        <div class="cart_body" style="min-height: 195px; max-height: 164px;overflow-y: scroll;">
                            <h5>Your booking with {{ ucfirst( $instructor->name ) }}:</h5>
                            <ul class="list-unstyled">
                                ...
                                <?php $existTest=false;?>
                                @if($search_step_2->test_schedule_date && $search_step_2->test_schedule_date!='undefined' && $checkTest)
                                        <?php
                                        $existTest=true;
                                        // Given time range
                                        ...
                                        ?>
                                <li>Driving Test Package: {{date('d-m-Y',strtotime($search_step_2->test_schedule_date))}}, {{$startTime->format('h:i a')}} pickup for {{$endTime->format('h:i a')}} a test start</li>
                                @endif

                                @if(count ($resultLesson)==0 && $existTest==false)
                ...
                <div class="col-md-6">
                    <div class="have_and_ac">
                        @if(auth()->user())
                            ...
                            <a href="{{URL::to ('/login')}}" class="btn btn-primary">My Account</a>
                        @else
                            <h4>Already have an account?</h4>
                            <a href="{{URL::to ('/login-demo')}}" class="btn btn-primary">Login</a>
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        <form class="row g-3" id="registerForm">
                            ...
                            @if(auth()->guest())
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control" type="text" name="name" @if($user) value="{{ $user->name }}" @endif placeholder="Your first name">
                                    <span class="error_global" id="name_error" style="color: red"></span>
                                </div>
                                <div class="col-md-6">
                                    <label for="lname" class="form-label">Last Name*</label>
                                    <input class="form-control" type="text" name="last_name" @if($user) value="{{ $user->lname }}" @endif placeholder="Your last name">
                                    <span class="error_global" id="last_name_error" style="color: red"></span>
                                </div>
                                <div class="col-md-6">
                                    <label for="phonenum" class="form-label">Mobile*</label>
                                    <input class="form-control" type="text" name="phone" @if($user) value="{{ $user->phone }}" @endif placeholder="+61XXXXXXX">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input class="form-control" type="text" @if($user) value="{{ $user->email }}" disabled @else name="email" @endif placeholder="Your email address">
