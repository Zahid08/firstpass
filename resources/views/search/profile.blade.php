        ...
        <div class="profile_info_box">
            <div class="profile_grey_box">
                <div class="profile_self_info_ctn">
                    <div class="profile_self_info">
                        <div class="pro_img_box">
                            <div class="pro_img_ctn">
                                ...
                                @else
                                    <img src="{{ url('assets/images/users/'.$instructor->avatar) }}" alt="user">
                                @endif
                            </div>
                            <div class="pro_ratings">
                                <img src="{!! url ('frontend_assets/images/profile-star-icon.svg') !!}" alt="">
                                <span class="start">{{ floor($avg_review->avg) }}</span>
                                <span class="ratings">{{ $avg_review->total }} ratings </span>
                            </div>
                        </div>
                        <div class="pro_info_ctn">
                            <div class="pro_info">
                                <h3 class="pro_info_name">{{$Iname}} </h3>
                                <img src="{!! url ('frontend_assets/images/male-icon.svg') !!}" alt="">
                            </div>
                            @if(@$instructor->wwccLicence->wwcc_licence_status == 'Approved')
                                <div class="pro_info_working">
                                    <img src="{!! url ('frontend_assets/images/child-icon.svg') !!}" alt="" />
                                    <span> Working with Children </span>
                                </div>
                            @endif
                            @if($instructor->user_meta->years_for_instructing!='')
                                <div class="pro_info_xp">
                                    <img src="{!! url ('frontend_assets/images/calender-icon.svg') !!}" alt="" />
                    ...
                    <div class="pro_btns">
                        <a href="{{ url('book-online/'.$search_id.'/instructor/'.$instructor->id) }}" class="btn btn-primary">Book Now</a>
                        <button type="button" data-id="{{$instructor->id}}" class="btn btn-primary active_btn av-btn" id="checkAvailability">Check Availability</button>
                    </div>
                </div>
            ...
            <div class="profile_bio d-block d-sm-none">
                <h3>{{ ucfirst($instructor->name) }}'s Bio</h3>
                <p>
                    {{ $instructor->user_meta->bio ==''? 'Not provided' : $instructor->user_meta->bio }}
                     <span onclick="toggleText2()" id="textButtonMore" class="show-btn">
                    ...
                    <span id="moreText">
{{--                        for you to choose from. Finding a quality driving instructor in--}}
{{--                        your area can be a difficult and consuming task, that’s why we--}}
{{--                        have built a comprehensive database of instructors for you to--}}
{{--                        choose from.choose from. Finding a qchoose from. Finding--}}
                        <br /><br />
...
{{--                        to help you pass your test the first tim ewith experienced--}}
{{--                        instructors, well-maintained end.--}}
                      </span>


                    <span class="show-btn" style="display: none" onclick="toggleText2()" id="textButtonLess" >
            ...
            <div class="review_info_box">
                ...
                foreach ($rating as $rate){
                    ...
                    <div class="review_bio_ctn">
                        <h4 class="review_name_heading">{{ ucfirst($rate->name. ' '. $rate->lname) }}’s Bio</h4>
                        <p> {{ $rate->review ==''? 'Not provided' : $rate->review }}</p>
                        <div class="review_bio_stars">
                            <ul class="d-flex">
                                @for($i=1; $i <= $rate->score; $i++)
                                <li><img src="{!! url ('frontend_assets/images/profile-star-icon.svg') !!}" alt=""></li>
                                @endfor
                            </ul>
                        </div>
                    </div>
                ...
                <?php } ?>
                <div class="see_more_review text-center">{!! $rating->links() !!}</div>
            </div>
            @if ($search->t_type == "auto")
            @if(@$instructor->user_vehicle_auto->vehicle_image != "")
                <div class="car_review_box">
                    <div class="car_review_head">
                        <h3 class="rc_heading">{{ ucfirst($instructor->name) }}’s Car</h3>
                        <span>Auto Transmission</span>
                    </div>
                    <div class="car_review_img">
                        <img class="img-fluid" src="{{ asset('assets/images/cars/'.$instructor->user_vehicle_auto->vehicle_image) }}" alt="">
                    </div>
                    <?php
                    ...
                    <div class="car_review_num">
                        <span class="car_review_code">{{ $instructor->user_vehicle_auto->registration_number }} </span>
                        <div class="car_review_codelist">
                            <span>{{ $instructor->user_vehicle_auto->ancap_rating }} Star</span>
                            <span>ANCAP</span>
                            <span>Rating</span>
                        </div>
                    </div>
    ...
    <section class="profile_map_sec">
        <div class="profile_map_box">
            <div class="profile_map_title">
                <h3>{{ ucfirst($instructor->name) }}’s services {{$user_region_ids}} suburbs</h3>
            </div>
                    ...
                    @if( $instructor->id != $user->id)
                        ...
                        <?php if($user->preferred_name!=""){ $name = $user->preferred_name; }
                            ...
                            <div class="suggested_instructor_box">
                                <a href="{{ url('search/'.$search_id.'/instructors/profile/'.$user->id) }}" class="text-decoration-none">
                                    ...
                                    <span class="instructor_title">{{ $name }}</span>
                                </a>
                            </div>
                    @endif
                @endforeach
