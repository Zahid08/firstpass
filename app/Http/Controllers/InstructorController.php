...

{
    ...
    function save_instructor_vehicle(Request $request){
        ...
        if($user_meta){
            ...
            if(!empty($_REQUEST['services_accreditation']) && is_array($_REQUEST['services_accreditation'])){
                $services_accreditation =$_REQUEST['services_accreditation'];
                if( in_array(1, $services_accreditation)){
                    $user_meta->update(
                        [
                            "check_test_with_cust" =>1,
                        ]
                    );
                } if( in_array(3, $services_accreditation)){
                    $user_meta->update(
                        [
                            "check_test_with_cust" =>0,
                        ]
                    );
                }
            }else{
        ...
        if($obj->save()){
            ...
            if($is_vehicle) {
                ...
                return response()->json(['success' => true, 'message' => 'Saved successfully and sent for approval.']);
    ...
    function save_my_documents(Request $request){
        ...
        return response()->json(['success' => true, 'message' => 'Documents saved successfully and sent for approval']);
    ...
    function get_instructor_calendar2(Request $request){
        ...
        if ($request->type=='test'){
            $hml = ' <select class="form-select availability_check choose_location mx-auto" aria-label="Default select example" id="test_location_date" name="schedule_date">
                    ...
                    for ($i=1; $i <=75 ; $i++)
                    {
...

                        {
                            if( $userWday[$toCheck]=='yes' ){
                                $hml.= "<option value='".$toUse."'>".$toShow."</option>";
                            }
                        }
                    }
            $hml .='</select>';
            ...
            $hml = '<select class="form-select availability_check" aria-label="Default select example" id="lesson_date" name="schedule_date">
...
//        ex
    ...
    function addEvent(Request $request){
        ...
        if($request->is_private == 0) {
            ...
            $appointment = Appointments::where('id', $request->appointment_id)->update([
...

    {
...

        {
            ...
            $data = Appointments::select('id', 'search_id', 'user_id', 'time_slot', 'lesson_hour', 'type', 'start_date', 'end_date', 'detail', 'address', 'note', 'is_private')->whereId($request->id)

                ->where('instructor_id', Auth::user()->id)

                ->first();
            if($data->search_id!=""){
               $get_type = Search::select('t_type')->where('id',$data->search_id)->first();

             $data['t_type'] = $get_type->t_type;
            } else{
                $data['t_type'] = '';
            }
            ...
            $data['pickuptime'] = date('h:i a', $pickup);

            $data['starttime'] = date('h:i a', strtotime($start_date));

            $data['endtime'] = date('h:i a', strtotime($data->end_date) + 1800);


            //$starttimestamp = strtotime($start_date);
            //$endtimestamp = strtotime($data->end_date);
            $datetime1 = new DateTime(date('H:i', $pickup));
            $datetime2 = new DateTime(date('H:i', strtotime($data->end_date)));
    ...
    public function buy_more_credit(Request $request){

        $report = \App\Search::where('learner_id',auth()->user()->id)
            ->first();
        ...
        $getCalcuatedData=$this->calcuateSum(1,70);
        ...
        $search =  Search::create(
            ...
            ['ip' => $ip, 't_type' => 'both','learner_id' => Auth::id(),'start_lesson'=>0,'start_package'=>0,'step_2'=>'["lesson","test"]','step_3'=>json_encode($search_data),'step_5'=>$report->step_5]

        );

        $search_id = $search->id;

        return redirect('learners/payment/'.$search_id.'?type=wallet');
    }
